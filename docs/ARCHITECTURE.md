...
### 2.3 原生个人主页与应用更新的网络边界

主页是纯 Compose 个人资料与入口页面，不在应用进程中请求或渲染博客内容。博客、GitHub 和项目入口只使用编译期已知的 HTTPS 地址交给系统浏览器。应用内唯一主动联网的功能是用户手动触发的更新模块，它只访问预配置的 HTTPS 清单和 APK，不能向 Repository、UseCase、播放器或其他音乐 feature 暴露通用网络能力。
...
flowchart LR
    ...
    UI --> SystemBrowser["系统浏览器 / 已知 HTTPS 个人链接"]
...
- 外部个人链接：只通过系统浏览器处理，应用不接收网页数据；
- 更新 I/O：受信任 HTTPS 主机上的清单与 APK。

任何在线音乐能力都不能借用外部链接或更新 I/O 边界。
...
四个顶层目的地固定为 Home、Browse、Library 和 Search。Home 使用原生 Compose 展示个人资料、空间入口、项目和动态，并保留设置入口；主页、Dock 和迷你播放器共享明确的 content inset。
